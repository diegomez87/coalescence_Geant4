...
G4TheoFSGenerator::G4TheoFSGenerator(const G4String& name)
    ...
    , theQuasielastic(0), fP0_d(0), fP0_dbar(0)
 {
...
G4HadFinalState * G4TheoFSGenerator::ApplyYourself(const G4HadProjectile & thePrimary, G4Nucleus &theNucleus)
{
  ...
  theParticleChange->SetStatusChange(stopAndKill);
	...
	//set p0 values for coalescence process
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
G4int G4TheoFSGenerator::FindPartner(const G4ThreeVector& p1, G4double m1, vector<std::pair<G4int, G4ThreeVector>> * Neutron, G4double m2, G4int charge) const
{
//
	...
	if(Model=="FTF"){
		...
		if(std::abs(thePrimary.GetDefinition()->GetPDGEncoding())==2212) {
			...
			if(thePrimary.GetDefinition()->GetPDGEncoding()==2212){
				...
				if(rs>6.){
					fP0_dbar = 179.6/(1+std::exp(6.77-std::log(0.001*rs)/0.41));//set p0 antideuteron
				} else fP0_dbar = 0.0;

				if(rs>1.8){
					fP0_d = 100.; //set p0 deuteron
				} else fP0_d = 0.0;
